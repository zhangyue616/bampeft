...
class BatchMolGraph:
    ...
    def __init__(self, mol_graphs, args: Namespace):
        ...
        fg_feature_extractor = getattr(args, 'fg_feature_extractor', None)
        if fg_feature_extractor is not None and getattr(args, 'use_fg_dora', False):
            use_shuffle = (
                getattr(args, 'fg_runtime_training', False)
                and getattr(args, 'ablation_method', None) == 'A5'
            )
            fg_rows = [
                fg_feature_extractor.compute_from_smiles(smiles, use_shuffle=use_shuffle)
                for smiles in self.smiles_batch
            ]
            self.fg_features = torch.FloatTensor(np.stack(fg_rows))
    ...
    def get_components(self) -> Tuple[torch.FloatTensor, torch.FloatTensor,
        ...
        and a list indicating the scope of the atoms (i.e. which molecules they belong to).
        """
        return self.f_atoms, self.f_bonds, self.a2b, self.b2a, self.b2revb, self.a_scope

    def get_fg_features(self) -> torch.FloatTensor:
        """
        Returns per-molecule FG descriptors without changing the 6-item get_components contract.
        """
        if self.fg_features is None:
            raise ValueError('FG features requested but args.fg_feature_extractor is not configured')
        return self.fg_features
